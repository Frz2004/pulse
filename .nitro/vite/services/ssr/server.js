...
function collectErrorText(error, seen = /* @__PURE__ */ new Set()) {
  if (error == null || seen.has(error)) return "";
  seen.add(error);
  if (typeof error === "string") return error;
  if (typeof error !== "object") return String(error);
  const record2 = error;
  const parts = [record2.name, record2.message, record2.stack].filter((value) => typeof value === "string");
  for (const key of ["cause", "runnerError", "error"]) {
    parts.push(collectErrorText(record2[key], seen));
  }
  if (Array.isArray(record2.errors)) {
    for (const item of record2.errors) parts.push(collectErrorText(item, seen));
  }
  return parts.filter(Boolean).join("\n");
}
function isStaleServerFunctionError(error) {
  return collectErrorText(error).includes("Invalid server function ID");
}
function staleServerFunctionResponse() {
...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./assets/server-ChSCHK5Z.js").then((n) => n.a5).then(
      (m) => m.default ?? m
    );
  }
...
export {
  server as default,
  isStaleServerFunctionError as i,
  renderErrorPage as r,
  staleServerFunctionResponse as s
};
